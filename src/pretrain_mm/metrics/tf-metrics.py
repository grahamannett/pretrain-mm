...
# FROM:
# https://github.com/Michael-Soloveitchik/CFID/blob/master/metrics.py
# Note: the values in pytorch differ quite a bit, seems like it comes from SVD calculations (on V)
...
def cfid(y_true, y_predict, x_true, embeding=no_embedding, estimator=sample_covariance):
    ...
    m_y_true_given_x_true = tf.reshape(m_y_true, (-1, 1)) + tf.matmul(c_y_true_x_true, A)
    m_y_predict_given_x_true = tf.reshape(m_y_predict, (-1, 1)) + tf.matmul(c_y_predict_x_true, A)

    c_y_true_given_x_true = c_y_true_y_true - tf.matmul(
        c_y_true_x_true, tf.matmul(inv_c_x_true_x_true, c_x_true_y_true)
    )
    c_y_predict_given_x_true = c_y_predict_y_predict - tf.matmul(
        c_y_predict_x_true, tf.matmul(inv_c_x_true_x_true, c_x_true_y_predict)
    )
    ...
    c_dist1 = tf.linalg.trace(
        tf.matmul(
            tf.matmul(c_y_true_x_true_minus_c_y_predict_x_true, inv_c_x_true_x_true),
